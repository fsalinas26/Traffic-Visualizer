...
private:
	...
	std::vector < std::vector<std::wstring> > queryurl;
...
public:
	...
	void		add_field(const PCHAR pszName, float fValue);
	void		add_field(const PCHAR pszName, int iValue);
	void		add_header(std::wstring header);
	void		add_query(std::wstring key, std::wstring value);

	DWORD		get_size(void) const;
	const char* get_data(void) const;
	std::vector<std::wstring> get_headers(void);
	std::vector<std::vector<std::wstring>> get_query(void);
};
...
private:
	...
	bool    send_request(std::wstring s_url, std::vector<BYTE>& bt_reply, const PWCHAR psz_type, const LPVOID p_data, const DWORD dw_data_len, std::vector<std::wstring> &headers);
...
public:
	...
	bool	get(std::wstring s_url, std::string& s_reply, std::vector < std::wstring > &headers);
	bool	post(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData);
	bool	post_h(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData);
	bool	get_h(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData);
