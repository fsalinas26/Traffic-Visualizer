...
std::string lw_http_tools::url_encode(std::string const& s_input)
{
	...
	for (size_t I = 0; I < s_input.length(); ++I)
	{
		...
		{
			char sz_buffer[4];
			sprintf_s(sz_buffer, xor_a("%%%02x"), ch & 0xFF);
			s_result += sz_buffer;
		}
		else
			s_result += ch;
...
void c_lw_httpd::fmt_out(const PCHAR pszFieldName, const PCHAR pszFmt, ...)
{
	...
	sprintf_s(szOut, xor_a("&%s=%s"), pszFieldName, sValueEncoded.c_str());
...
{
	fmt_out(pszName, (PCHAR)xor_a("%s"), pszValue);
}

void c_lw_httpd::add_header(std::wstring header)
{
...
{
	fmt_out(pszName, (PCHAR)xor_a("%f"), fValue);
}

void c_lw_httpd::add_field(const PCHAR pszName, int iValue)
{
	fmt_out(pszName, (PCHAR)xor_a("%i"), iValue);
}
...
std::vector<std::wstring> c_lw_httpd::get_headers(void)
{
	return headers;
}
...
{
	char buffer[32];
	TCHAR* szUrl = new TCHAR[255];
	DWORD dwLen = sizeof(TCHAR) * 255;
	WinHttpQueryOption(h_request, WINHTTP_OPTION_URL, szUrl, &dwLen);
	std::cout << "URL: " << std::string(URL.begin(),URL.end()).c_str() << '\n';
	std::string s(reinterpret_cast<const char*>(szUrl),dwLen);
	std::cout << "URL_full: " << s<< '\n';
	std::cout << "OUTGOING HTTP REQUEST\n" << std::string(30, '*') << '\n';
	std::cout << "HEADERS\n"<< queryHeaders(h_request) << '\n';
	std::cout << "DATA size: " << std::string(reinterpret_cast<const char*>(p_data)).c_str() << "\n";// , dw_data_len).c_str() << '\n';
...
bool c_lw_http::get(const std::wstring sURL, std::string& s_reply,std::vector<std::wstring> &headers)
{
	std::vector< BYTE > bt_reply;
...
{
	return send_request(s_url, btReply, L"GET", nullptr, 0);
}
...
bool c_lw_http::post(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData)
{
	...
	bool bResult = send_request(sURL, btReply, L"POST",
...
bool c_lw_http::post_h(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData)
{
	std::vector< BYTE > btReply;

	bool bResult = send_request(sURL, btReply, L"POST",
		(LPVOID)(PostData.get_data()), PostData.get_size(), PostData.get_headers());
...
bool c_lw_http::get_h(std::wstring sURL, std::string& sReply, c_lw_httpd& PostData)
{
	std::vector< BYTE > btReply;

	bool bResult = send_request(sURL, btReply, L"GET",
		(LPVOID)(PostData.get_data()), PostData.get_size(), PostData.get_headers());
...
{
	return send_request(sURL, btReply, L"POST",
		LPVOID(PostData.get_data()), PostData.get_size());
}
